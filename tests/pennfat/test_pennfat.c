...
void test_k_write_read(void)
{
    ...
    TEST_CHECK(mount(test_fs_name) == 0);

    int fd = k_open("a", F_WRITE);
    TEST_CHECK(fd >= 0);

    char *str = "hello world";
    int len = strlen(str);
    TEST_CHECK(k_write(fd, str, strlen(str)) == len);

    // reading from the same position should yield nothing
    char out[32] = "";
    int bytes_read = k_read(fd, 32, out);
    ...
    TEST_CHECK(k_close(fd) == 0);
    TEST_CHECK(unmount() == 0);
}
...
void test_k_lseek_past_end(void)
{
    ...
    char out[200] = "";
    int bytes_read = k_read(fd, 200, out);
...
void test_k_lseek_various(void)
{
    ...
    // Try to read 1 byte
    TEST_CHECK(k_read(fd, 1, out) == 0);
    TEST_CHECK(out[0] == 'l');

    // Try to read 100 bytes
    TEST_CHECK(k_read(fd, 100, out) == 0);
    TEST_CHECK(out[0] == 'l');
...
void test_k_many_opens(void)
{
    ...
    {
        fds[i] = k_open("a", F_WRITE);
        TEST_CHECK(fds[i] >= 0);
    }

    for (int i = 0; i < 1000; i++)
    {
        TEST_CHECK(k_close(fds[i]) == 0);
    }
...
void test_big_write_and_read(void)
{
    ...
    TEST_CHECK(k_write(fd, str, 1300) == 1300);
    TEST_CHECK(k_lseek(fd, 0, F_SEEK_CUR) == 1300);
    TEST_CHECK(k_lseek(fd, 0, F_SEEK_SET) == 0);

    char out[1301] = "";
    out[1300] = '\0';
    TEST_CHECK(k_read(fd, 1300, out) == 1300);
    TEST_CHECK(strcmp(out, str) == 0);
...
void test_stdin_stdout_stderr(void)
{
    ...
    int bytes_written;
    bytes_written = k_write(STDIN_FD, "hello", 5);
    ...
    bytes_written = k_write(STDOUT_FD, "hello\n", 6);
    TEST_CHECK(bytes_written == 6);
    bytes_written = k_write(STDERR_FD, "hello\n", 6);
    TEST_CHECK(bytes_written == 6);
    ...
    char out[100];
    int bytes_read = k_read(STDIN_FD, 100, out);
    ...
    bytes_read = k_read(STDOUT_FD, 100, out);
    TEST_CHECK(bytes_read == 6);
    TEST_CHECK(strcmp(out, "hello\n") == 0);

    bytes_read = k_read(STDERR_FD, 100, out);
    TEST_CHECK(bytes_read == 6);
    TEST_CHECK(strcmp(out, "hello\n") == 0);
...
void test_operating_on_special_fds(void)
{
    ...
    TEST_CHECK(k_close(STDIN_FD) == EK_CLOSE_SPECIAL_FD);
    TEST_CHECK(k_close(STDOUT_FD) == EK_CLOSE_SPECIAL_FD);
    TEST_CHECK(k_close(STDERR_FD) == EK_CLOSE_SPECIAL_FD);
...
void test_k_ls_on_one_file(void)
{
    ...
    // 31 char filename
    int fd = k_open("this-is-aagam-s-first-ever-file", F_WRITE);
    TEST_CHECK(fd >= 0);
    TEST_MSG("Produced fd: %d", fd);

    TEST_CHECK(k_write(fd, "hello", 5) == 5);
    TEST_CHECK(k_close(fd) == 0);

    TEST_CHECK(k_ls("this-is-aagam-s-first-ever-file") == 0);

    // get input that the output is correct (using k_write and k_read for fun)
    char out[2];
    out[1] = '\0';
    k_write(STDOUT_FD, "Does k_ls work? [y/N] ", 22);
    int bytes_read = k_read(STDIN_FD, 1, out);
...
void test_k_ls_after_unlink(void)
{
    ...
    TEST_CHECK(k_unlink("this-is-aagam-s-first-ever-file") == 0);
    
    TEST_CHECK(k_ls(NULL) == 0);

    // get input that the output is correct (using k_write and k_read for fun)
    char out[3];
    out[2] = '\0';
    k_write(STDOUT_FD, "Did k_ls not output anything? [y/N] ", strlen("Did k_ls not output anything? [y/N] "));
    int bytes_read = k_read(STDIN_FD, 2, out);
...
void test_k_ls_on_non_existent_file(void)
{
    ...
    int status = k_ls("this-is-aagam-s-first-ever-file");
