...
int main(void) {
	while (true) {
		...
		if (strcmp(tokens[0], "mkfs") == 0) {
			...
			int unlink_status = k_unlink(&fs, tokens[1]);
			if (unlink_status < 0) {
			...
			if ((write_mode || append_mode) && n_tokens == 3) {
				...
				int fd = k_open(&fs, output_file, mode);
				if (fd < 0) {
				...
				fprintf(stderr, "Enter text (Ctrl+D to end):\n");
				char buffer[1024];
				int bytes_read;
				
				while ((bytes_read = k_read(&fs, STDIN_FD, sizeof(buffer), buffer)) > 0) {
					if (k_write(&fs, fd, buffer, bytes_read) < 0) {
			...
			// Case 2: cat FILE1 FILE2 ... [-w|-a OUTPUT_FILE]
				...
				if (output_file != NULL) {
					...
					out_fd = k_open(&fs, output_file, mode);
					if (out_fd < 0) {
						fprintf(stderr, "cat: failed to open output file with error code %d\n", out_fd);
						goto cleanup_tokens;
					}
				...
				for (size_t i = 1; i < n_tokens; i++) {
					...
					// Open current file
					int in_fd = k_open(&fs, tokens[i], F_READ);
					if (in_fd < 0) {
					...
					char buffer[1024];
					int bytes_read;
					while ((bytes_read = k_read(&fs, in_fd, sizeof(buffer), buffer)) > 0) {
						// Write to output file if specified, otherwise to stdout
						int write_fd = (out_fd >= 0) ? out_fd : STDOUT_FD;
						if (k_write(&fs, write_fd, buffer, bytes_read) < 0) {
			...
			// List specific file
				...
				int ls_status = k_ls(&fs, tokens[1]);
				if (ls_status < 0) {
